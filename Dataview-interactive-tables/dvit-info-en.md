...
`"entries on page": <number>` — describes how many entries shoul be on one page. If you skip this option, there will be no pagination.
...
`image: true` — property will be rendered as impage. For this it must contain either external url link or wikilink to the local image.

`width: <number>` — sets the width of the image.

`fuzzySearch: true` — allows fuzzy search in filtering modal.

`span: true` — wraps property values into span tags with unical class names. It allows to use css for adding different colors for different values.
...
```
Slider requires two properties: maximal value and current value. If max value do not exist, you can set it up by clicking on the slider cell. Double clicking on the slider allow you to change the value as a number.
...
You can also use css to change colors of specific properties values. To do this you must add the `span: true` option to the property object. Then you need to create the css snippet. For example, you have the property "Status" with the possible values "in progress" and "completed" and you want "in progress" to be red and "completed" to be green. You can use the css like this:
